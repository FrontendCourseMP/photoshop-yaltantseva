...
export function encodeGB7(imageData: ImageData): Uint8Array {
  ...
  buffer[5] = hasMask ? 0x01 : 0x00; // флаг маски
  ...
  // Пиксели — RGB → оттенки серого → 7 бит + маска в бит 7
  for (let i = 0; i < width * height; i++) {
    const idx = i * 4;
    const r = data[idx];
    const g = data[idx + 1];
    ...
    const grayBits = Math.round((gray / 255) * 127) & 0x7f;

    // Бит 7: маска прозрачности (1 = непрозрачный, 0 = прозрачный)
    const maskBit = a >= 128 ? 0x80 : 0x00;

    buffer[12 + i] = grayBits | maskBit;
  }
